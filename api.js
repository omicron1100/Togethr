...
exports.setApp = function (app, client) {
  ...
  app.post("/api/viewlikedevents", async (req, res, next) => {
    ...
    try {
      while (i < len) {
        await Event.find({ _id: LikedEvents[i] })
          .then((event) => {
            if (!event.length) {
              i++;
            } else {
  ...
  app.post("/api/viewattendingevents", async (req, res, next) => {
    ...
    if (req.body.AttendingEvents.length === 0) {
      return res
        .status(301)
        .json({ warning: "please select an event to attend" });
    }
    var i = 0;
    try {
      while (i < len) {
        await Event.find({ _id: Attending[i] })
          .then((event) => {
            if (!event.length) {
              i++;
            } else {
    ...
    try {
      ...
      const result = await User.findOneAndUpdate(
        ...
        if (!users) {
          return res
            .status(301)
            .json({ warning: "No matching username exists." });
        }
        ...
        res.send(
          "Email containing your password reset code has been sent to the associated email address."
        );
      });
    } catch (error) {
      console.log(error.message);
